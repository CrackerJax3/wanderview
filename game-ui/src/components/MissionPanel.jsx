  ...
  return (
    <div className="mission-panel">
      <div className="mission-card">
        <div
          className="mission-header interactive"
          onClick={() => setMinimized((prev) => !prev)}
        >
          <div className={`mission-status-dot ${completed ? 'completed' : ''}`} />
          <span className="mission-title">
            {completed ? 'Completed' : 'To Do List'}
          </span>
          <span className={`mission-toggle ${minimized ? 'collapsed' : ''}`}>
            {minimized ? '\u25B6' : '\u25BC'}
          </span>
        </div>
        {minimized ? (
          <div className="mission-name-mini">{mission.title}</div>
        ) : (
          <>
